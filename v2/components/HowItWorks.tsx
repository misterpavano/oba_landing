const steps = [
  { num: "01", title: "I Audit Your Workflow", desc: "We spend time together — in person or on a call. I learn how your business actually runs: where time gets wasted, what's repetitive, where the bottlenecks are. No assumptions.", accent: 'var(--color-energy-secondary)' },
  ...
  { num: "03", title: "You Own Everything", desc: "Every system, document, and login belongs to you. I teach you as we build so you're never dependent on me. Monthly check-ins available if you want a partner as things evolve.", accent: 'var(--color-energy-secondary)' },
];
    ...
    <section id="how-it-works" style={{ background: 'var(--color-wireframe)', color: 'var(--color-base)', padding: '6rem 1.5rem' }}>
      <div style={{ maxWidth: '960px', margin: '0 auto' }}>
        ...
        <h2 style={{ fontFamily: 'var(--font-sans)', fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 900, lineHeight: 1, marginBottom: '4rem' }}>
          Three steps.<br />
          {/* Headline accent → cyan, pink reserved for CTAs */}
          <span style={{ color: 'var(--color-energy-secondary)' }}>Zero fluff.</span>
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '2rem' }}>
          {steps.map((step) => (
            <div key={step.num} className="wireframe-card" style={{ borderColor: '#333', background: 'transparent' }}>
              {/* Step numbers → cyan, reduced opacity so title leads */}
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '3rem', fontWeight: 900, color: step.accent, opacity: 0.6, marginBottom: '1.5rem' }}>{step.num}</div>
